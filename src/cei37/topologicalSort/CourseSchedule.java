...
public class CourseSchedule {
	...
	//this is working fine, let's see a different approach
    public boolean canFinish(int numCourses, int[][] prerequisites) {
        ...
        for (int n=0; n<numCourses; n++) {
        	if (dfs(graph, n, visited, onStack)) {
    ...
    public boolean dfs(Map<Integer, List<Integer>> graph, int node, Set<Integer> visited, Set<Integer> onStack) {
    	visited.add(node);
    	onStack.add(node);
    	if (graph.get(node) != null) {
    		for (Integer n : graph.get(node)) {
        		if (!visited.contains(n) && dfs(graph, n, visited, onStack)) {
        			return true;
