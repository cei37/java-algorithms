...
There is a binary tree with N nodes. You are viewing the tree from its left side and can 
see only the leftmost nodes at each level. Return the number of visible nodes.
Note: You can see only the leftmost nodes, but that doesn't mean they have to be 
left nodes. The leftmost node at a level could be a right node.
Signature
...
public class NumberOfVisibleNodes {
	...
	int maxLevel = 0;
	void visibleNodes(Node node, int level) {
		...
		visibleNodes(node.left, level + 1);
